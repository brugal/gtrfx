...
#if 0
  printf("Tuneit:  Note %-2s (%8.3fHz): %+3.f cents (%8.3fHz)      \r",
	 notes[note], nfreq, 1200*(log(freq/nfreq)/LOG_2), freq);
#endif
  if (nfreq > 0.01) {
	  printf("Tuneit:  Note %-2s (%8.3fHz): %+3.f cents (%8.3fHz)\n",
			 notes[note], nfreq, 1200*(log(freq/nfreq)/LOG_2), freq);
  }
...
schmittS16LE (int nframes, signed short int *indata)
{
  ...
  for (i=0; i<nframes; i++) {
	  //FIXME testing noise gate
#define THRESHOLD 20000
	  if (indata[i] > THRESHOLD  ||  indata[i] < THRESHOLD)
...
{
	schmittFloat(nframes, indata);
}

void tuneit_runi (int nframes, signed short int *indata)
{
	schmittS16LE(nframes, indata);
}
